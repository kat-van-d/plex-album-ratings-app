...
if not st.session_state.user_id:
    ...
    if st.session_state.selected_album_id:
        ...
        album_browser()
    for key in ['access_token', 'refresh_token', 'user_id', 'user_email', 'display_name', 'selected_album_id']:
...
def render_artwork(artwork_url, large=False):
    if artwork_url:
        ...
        font_size = 64 if large else 48
        st.markdown(
            f'''<div style="aspect-ratio:1/1;background-color:#2b2b2b;display:flex;align-items:center;justify-content:center;border-radius:8px;font-size:{font_size}px;margin-bottom:0.5rem;">🎵</div>''',
            unsafe_allow_html=True,
        )
...
def login_page():
    st.title('🎵 Plex Album Ratings')
    st.write('Sign in to browse the Plex music library, rate albums, and save your notes.')
        ...
        if submitted:
            try:
                ...
                profile_response = (
                    ...
                    .eq('user_id', response.user.id).limit(1).execute()
                )
                if profile_response.data:
                    st.session_state.display_name = profile_response.data[0].get('display_name')
                st.rerun()
            except Exception as exc:
...
def get_all_albums():
    ...
    while True:
        response = (
            ...
            .select('album_id,title,year,studio,summary,artwork_url,artist_id,artists(name)')
...
def get_user_reviews():
    response = (
        supabase.table('album_reviews').select('album_id,rating,notes')
...
def get_album_detail(album_id):
    response = (
        supabase.table('albums')
        .select('album_id,title,year,studio,summary,artwork_url,rating,plex_user_rating,artist_id,artists(name)')
        .eq('album_id', album_id).limit(1).execute()
    )
...
def get_tracks(album_id):
    response = (
        ...
        .select('track_id,track_number,title,duration_human,audio_codec,bitrate,view_count')
        .eq('album_id', album_id).order('track_number').execute()
    )
    return response.data or []
...
def get_my_review(album_id):
    response = (
        supabase.table('album_reviews').select('review_id,rating,notes')
        .eq('album_id', album_id).eq('user_id', st.session_state.user_id)
        .limit(1).execute()
    )
...
def sidebar_filters(albums):
    with st.sidebar:
        st.header('Music Library')
        user_label = st.session_state.display_name or st.session_state.user_email or 'Signed-in user'
        st.caption(f'Signed in as {user_label}')
        if st.button('Sign out', use_container_width=True):
            sign_out()
        st.divider()
        search = st.text_input('Search albums or artists')
        years = sorted({row.get('year') for row in albums if row.get('year') is not None})
        year_choice = st.selectbox('Year', ['All'] + [str(year) for year in years])
...
def album_browser():
    set_supabase_session()
    albums = get_all_albums()
    search, year_choice, sort_choice, only_unrated = sidebar_filters(albums)
    st.title('Albums')

    reviews = get_user_reviews()
    reviewed_by_album = {review['album_id']: review for review in reviews}
    rows = albums.copy()
    ...
    if search:
        ...
        filtered_rows = []
        for row in rows:
            artist_name = (relation_one(row.get('artists')).get('name') or '')
            album_title = row.get('title') or ''
            if search_lower in album_title.lower() or search_lower in artist_name.lower():
                filtered_rows.append(row)
        rows = filtered_rows

    if year_choice != 'All':
        selected_year = int(year_choice)
        rows = [row for row in rows if row.get('year') == selected_year]

    if only_unrated:
        rows = [row for row in rows if row['album_id'] not in reviewed_by_album]

    if sort_choice == 'Artist / Album':
        rows.sort(key=lambda row: (((relation_one(row.get('artists')).get('name') or '').lower()), ((row.get('title') or '').lower())))
    elif sort_choice == 'Album title':
        rows.sort(key=lambda row: (row.get('title') or '').lower())
    elif sort_choice == 'Year (newest)':
        rows.sort(key=lambda row: row.get('year') or 0, reverse=True)
    elif sort_choice == 'Year (oldest)':
        rows.sort(key=lambda row: row.get('year') or 9999)
    ...
    columns_per_row = 4
    for row_start in range(0, len(rows), columns_per_row):
        columns = st.columns(columns_per_row)
        for offset, row in enumerate(rows[row_start:row_start + columns_per_row]):
            with columns[offset]:
                render_artwork(row.get('artwork_url'))
                artist = relation_one(row.get('artists')).get('name') or 'Unknown artist'
                title = row.get('title') or 'Untitled'
                year = row.get('year')
                st.markdown(f'**{title}**')
                detail = artist + (f' • {year}' if year else '')
                st.caption(detail)
                review = reviewed_by_album.get(row['album_id'])
                if review and review.get('rating') is not None:
                    st.caption(f"Your rating: {review['rating']}/5")
                else:
...
def album_detail(album_id):
    ...
    artist = relation_one(album.get('artists')).get('name') or 'Unknown artist'

    header_left, header_right = st.columns([1, 3])
    with header_left:
        render_artwork(album.get('artwork_url'), large=True)
    with header_right:
        st.title(album.get('title') or 'Untitled')
        st.subheader(artist)
        metadata = []
        if album.get('year'):
            metadata.append(str(album['year']))
        if album.get('studio'):
            metadata.append(album['studio'])
        if metadata:
    ...
    with left:
        ...
        if tracks:
            ...
            display_columns = [c for c in ['track_number', 'title', 'duration_human', 'audio_codec', 'bitrate'] if c in track_df.columns]
            track_df = track_df[display_columns].rename(columns={
                'track_number': '#',
                'title': 'Track',
            ...
            st.info('No tracks found for this album.')

    with right:
        st.header('Your review')
        current = get_my_review(album_id)
        current_rating = current.get('rating')
        default_rating = float(current_rating) if current_rating is not None else 3.0
        current_notes = current.get('notes') or ''

        with st.form(f'review_{album_id}'):
            rating = st.slider('Rating', min_value=0.0, max_value=5.0, value=default_rating, step=0.5)
            notes = st.text_area('Notes', value=current_notes, height=180)
    ...
    reviews = get_community_reviews(album_id)
    if not reviews:
        st.caption('No reviews yet.')
    else:
        for review in reviews:
            profile = relation_one(review.get('profiles'))
            name = profile.get('display_name') or 'User'
            rating = review.get('rating')
            notes = review.get('notes')
            with st.container(border=True):
                if rating is not None:
                    st.markdown(f'**{name} — {rating}/5**')
                else:
                    st.markdown(f'**{name}**')
                if notes:
                    st.write(notes)
